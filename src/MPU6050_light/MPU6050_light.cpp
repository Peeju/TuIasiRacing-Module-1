...
angleAccY = atan2(accZ, sqrt(accY*accY + accX*accX))*RAD_2_DEG;
angleAccZ = atan2(accX, sqrt(accZ*accZ + accY*accY))*RAD_2_DEG;
  ...
  angleZ = wrap(filterGyroCoef*(angleAccZ + wrap(angleZ + gyroZ*dt - angleAccZ, 180)) + (1.0-filterGyroCoef)*angleAccY, 180);
